...
| Post Edit | Per-block language and voice       | Toggle ‘Customize’ in the block inspector to override the post’s language and voice for one block. Stored as block attributes and rendered as `data-beyondwords-language` / `data-beyondwords-voice-id` on the block, in the HTML sent to the API only — the front end is unchanged. A container block’s choice cascades to the blocks inside it, which may override it again. |
