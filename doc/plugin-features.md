...
| Post Edit | Per-block language and voice       | Toggle ‘Customize’ in the block inspector to override the post’s language and voice for one block, starting from whatever it already inherits. A container’s choice cascades to the blocks inside it. Sent to the API only — the front end is unchanged. |
