 ...
 */
class TitleVoiceTest extends TestCase
{
    public function setUp(): void
    {
        parent::setUp();

        delete_option(TitleVoice::OPTION_NAME);
    ...
    public function init_registers_admin_init_hook(): void
    {
        ...
        $this->assertEquals(
            10,
            has_action('admin_init', [TitleVoice::class, 'addSetting']),
    ...
    public function init_registers_pre_update_option_hook(): void
    {
        ...
        $this->assertTrue(
            has_action('pre_update_option_' . TitleVoice::OPTION_NAME) !== false,
    ...
    public function addSetting_registers_setting_correctly(): void
    {
        ...
        TitleVoice::addSetting();

        $this->assertArrayHasKey(
            TitleVoice::OPTION_NAME,
            $wp_registered_settings,
            'Should register the title voice setting'
        );

        $setting = $wp_registered_settings[TitleVoice::OPTION_NAME];
    ...
    public function addSetting_registers_settings_field(): void
    {
        ...
        $field = $wp_settings_fields['beyondwords_voices']['voices']['beyondwords-title-voice'];
        $this->assertSame('Title voice', $field['title']);
        $this->assertSame([TitleVoice::class, 'render'], $field['callback']);
    }
    ...
    public function render_outputs_select_element(): void
    {
        ...
        $select = $crawler->filter('select#' . TitleVoice::OPTION_NAME);
    ...
    public function render_outputs_wrapper_div(): void
    {
        ...
        $crawler = new Crawler($html);
        $wrapper = $crawler->filter('div.beyondwords-setting__title-voice');
    ...
    public function render_preselects_current_option(): void
    {
        ...
        $voices = TitleVoice::getOptions();
        if (count($voices) > 0) {
            $selectedVoiceId = $voices[0]['value'];
            update_option(TitleVoice::OPTION_NAME, $selectedVoiceId);
    ...
    public function integration_full_workflow(): void
    {
        ...
        if (count($voices) > 0) {
            ...
            update_option(TitleVoice::OPTION_NAME, $voiceId);

            // Retrieve and verify
            $savedVoiceId = get_option(TitleVoice::OPTION_NAME);
    ...
    {
        $reflection = new \ReflectionClass(TitleVoice::class);
        $parent = $reflection->getParentClass();

        $this->assertNotFalse($parent, 'TitleVoice should extend a parent class');
        $this->assertSame(
            'Beyondwords\Wordpress\Component\Settings\Fields\Voice\Voice',
            $parent->getName(),
            'TitleVoice should extend Voice class'
        );
    }
     ...
     * @test
     */
    public function title_voice_has_different_option_name_than_body_voice(): void
    {
        $this->assertNotSame(
            'beyondwords_project_body_voice_id',
            TitleVoice::OPTION_NAME,
            'TitleVoice should have different option name than BodyVoice'
        );
    }
}
