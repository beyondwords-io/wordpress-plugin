 ...
 */
class BodyVoiceSpeakingRateTest extends TestCase
{
    public function setUp(): void
    {
        parent::setUp();

        delete_option(BodyVoiceSpeakingRate::OPTION_NAME);
    }
    ...
    public function option_name_constant_is_correct(): void
    {
        $this->assertSame(
            ...
            BodyVoiceSpeakingRate::OPTION_NAME,
    ...
    public function init_registers_admin_init_hook(): void
    {
        ...
        $this->assertEquals(
            10,
            has_action('admin_init', [BodyVoiceSpeakingRate::class, 'addSetting']),
    ...
    public function init_registers_pre_update_option_hook(): void
    {
        ...
        $this->assertTrue(
            has_action('pre_update_option_' . BodyVoiceSpeakingRate::OPTION_NAME) !== false,
    ...
    public function addSetting_registers_setting_correctly(): void
    {
        ...
        BodyVoiceSpeakingRate::addSetting();

        $this->assertArrayHasKey(
            BodyVoiceSpeakingRate::OPTION_NAME,
            $wp_registered_settings,
            'Should register the body voice speaking rate setting'
        );

        $setting = $wp_registered_settings[BodyVoiceSpeakingRate::OPTION_NAME];
    ...
    public function addSetting_registers_settings_field(): void
    {
        ...
        $field = $wp_settings_fields['beyondwords_voices']['voices']['beyondwords-body-speaking-rate'];
        $this->assertSame('Body voice speaking rate', $field['title']);
        $this->assertSame([BodyVoiceSpeakingRate::class, 'render'], $field['callback']);
    }
    ...
    {
        update_option(BodyVoiceSpeakingRate::OPTION_NAME, 100);

        $html = $this->captureOutput(function () {
            BodyVoiceSpeakingRate::render();
        });

        $crawler = new Crawler($html);

        $input = $crawler->filter('input[type="range"]#' . BodyVoiceSpeakingRate::OPTION_NAME);
    ...
    {
        update_option(BodyVoiceSpeakingRate::OPTION_NAME, 150);
    ...
    public function render_outputs_wrapper_div(): void
    {
        ...
        $crawler = new Crawler($html);
        $wrapper = $crawler->filter('div.beyondwords-setting__body-speaking-rate');
    ...
    {
        update_option(BodyVoiceSpeakingRate::OPTION_NAME, $rate);
    ...
    public function integration_full_workflow(): void
    {
        ...
        // Set a speaking rate
        update_option(BodyVoiceSpeakingRate::OPTION_NAME, 125);

        // Retrieve and verify
        $savedRate = get_option(BodyVoiceSpeakingRate::OPTION_NAME);
