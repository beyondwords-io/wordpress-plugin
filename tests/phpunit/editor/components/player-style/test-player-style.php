...
class PostPlayerStyleTest extends TestCase
{
     ...
     * @test
     */
    public function rest_api_init_callback_registers_player_styles_route()
    {
        global $wp_rest_server;
        $wp_rest_server = new \WP_REST_Server();

        add_action('rest_api_init', [PlayerStyle::class, 'rest_api_init_callback']);
        ...
        foreach (array_keys($routes) as $route) {
            if (str_contains($route, '/player-styles')) {
                $found = true;
                break;
            }
        }
        $this->assertTrue($found, 'Player styles REST route should be registered');

        remove_action('rest_api_init', [PlayerStyle::class, 'rest_api_init_callback']);
    }

    /**
     * @test
     */
    public function player_styles_rest_api_response_returns_rest_response()
    {
        $response = PlayerStyle::player_styles_rest_api_response();
