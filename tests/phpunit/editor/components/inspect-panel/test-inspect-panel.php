...
use BeyondWords\Editor\Components\InspectPanel;
...
class InspectTest extends TestCase
{
    ...
    /**
     * @var \BeyondWords\Editor\Components\InspectPanel
     */
    private $_instance;
     ...
     * @test
     */
    public function save_with_remove()
    {
    ...
    public function hide_meta_box_adds_inspect_id_to_hidden_list()
    {
        ...
        $result = InspectPanel::hide_meta_box($hidden);

        $this->assertContains('beyondwords__inspect', $result);
        $this->assertContains('some_other_metabox', $result);
    }

    /**
     * @test
     */
    public function rest_api_init_callback_registers_inspect_route()
    {
        global $wp_rest_server;
        $wp_rest_server = new \WP_REST_Server();

        add_action('rest_api_init', [InspectPanel::class, 'rest_api_init_callback']);
        ...
        foreach (array_keys($routes) as $route) {
            if (str_starts_with($route, '/beyondwords/v1/projects/') && str_contains($route, '/content/')) {
                $found = true;
                break;
            }
        }
        $this->assertTrue($found, 'Inspect REST route should be registered');

        remove_action('rest_api_init', [InspectPanel::class, 'rest_api_init_callback']);
    }

    /**
     * @test
     */
    public function rest_api_response_rejects_non_numeric_project_id()
    {
        $request = new \WP_REST_Request('GET', '/beyondwords/v1/projects/abc/content/xyz');
        $request->set_url_params(['projectId' => 'abc', 'beyondwordsId' => 'xyz']);

        // rest_ensure_response() returns a WP_Error directly when given one.
     ...
     * @test
     */
    public function rest_api_response_rejects_empty_content_id()
    {
        $request = new \WP_REST_Request('GET', '/beyondwords/v1/projects/123/content/');
        $request->set_url_params(['projectId' => '123', 'beyondwordsId' => '']);
