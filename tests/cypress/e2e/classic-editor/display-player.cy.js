context( 'Classic Editor: Display Player', () => {
  ...
  postTypes.filter( x => x.priority ).forEach( postType => {
    it( `${postType.name} can hide and reshow the player`, () => {
      ...
      cy.classicSetPostTitle( `I can toggle player visibility for a ${postType.name}` )
      ...
      cy.get( '#sample-permalink' ).click().wait( 1000 )
      ...
      cy.get( 'input#beyondwords_display_player' ).should( 'not.be.checked' )
