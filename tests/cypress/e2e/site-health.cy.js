context( 'Site Health', () => {
  ...
  it( 'has BeyondWords site health info', () => {
    ...
    cy.get( '#health-check-accordion-block-beyondwords' )
      .within( () => {
        // Plugin version
        cy.get( 'tr' ).eq( 0 ).within( () => {
          cy.get( 'td' ).eq( 0 ).should( 'have.text', 'Plugin version' )
          cy.get( 'td' ).eq( 1 ).invoke( 'text' ).should( 'match', semverRegex )
        } )
        // REST API URL
        cy.get( 'tr' ).eq( 1 ).within( () => {
          cy.get( 'td' ).eq( 0 ).should( 'have.text', 'REST API URL' )
          cy.get( 'td' ).eq( 1 ).should( 'have.text', Cypress.env( 'apiUrl' ) )
        } )
        // Communication with REST API
        cy.get( 'tr' ).eq( 2 ).within( () => {
          cy.get( 'td' ).eq( 0 ).should( 'have.text', 'Communication with REST API' )
          cy.get( 'td' ).eq( 1 ).should( 'have.text', 'BeyondWords API is reachable' )
        } )
        // Compatible post types
        cy.get( 'tr' ).eq( 3 ).within( () => {
          cy.get( 'td' ).eq( 0 ).should( 'have.text', 'Compatible post types' )
          cy.get( 'td' ).eq( 1 ).should( 'have.text', 'post, page, cpt_active, cpt_inactive' )
        } )
        // Incompatible post types
        cy.get( 'tr' ).eq( 4 ).within( () => {
          cy.get( 'td' ).eq( 0 ).should( 'have.text', 'Incompatible post types' )
        ...
        cy.get( 'tr' ).eq( 7 ).within( () => {
          cy.get( 'td' ).eq( 0 ).should( 'have.text', 'Include title in audio' )
