  ...
  on( 'task', {
    ...
    async 'reset'() {
      if ( process.env.CI ) {
        ...
        await exec( 'wp plugin activate speechkit Basic-Auth cpt-active cpt-inactive cpt-unsupported beyondwords-filter-e2e-testing' )
      } else {
        ...
        await exec( `yarn wp-env run tests-cli wp plugin activate speechkit Basic-Auth cpt-active cpt-inactive cpt-unsupported beyondwords-filter-e2e-testing` )
      }
      return null
    },
